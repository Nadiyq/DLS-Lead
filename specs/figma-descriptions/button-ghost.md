...
Category: action
Figma component set: button-ghost
React: <Button variant="ghost">
...
Figma: https://www.figma.com/design/oKckNLXwLm5fYEmdXaynYB/DLS-Lead?node-id=6459-2714
...
This Figma component set uses: GHOST BUTTON.
Code mapping is fixed to `variant="ghost"`, so hover/pressed use
...
## Purpose
Borderless low-emphasis direct action control. Use it for tertiary,
inline, toolbar, or contextual actions that should stay available
without adding a filled, outlined, tonal, or dashed container.

## Use when
- The action should be visible but lower emphasis than filled,
  outline, soft, or dotted.
- The action is tertiary, contextual, or repeated in a dense area.
- The surrounding layout already provides enough structure and the
  button does not need its own border or surface at rest.
...
- Primary/secondary action rows where hierarchy should be expressed
  as filled + outline.
- Rows where two ghost buttons are used to represent primary and
  secondary actions.
- Supporting actions that need a visible tonal surface -> use soft.
- Optional placeholder-like actions that need a dashed boundary -> use dotted.
...
- This Figma component set covers ghost buttons only.
- Icon-only ghost buttons should use the dedicated icon-ghost
...
5. Borderless rest state - transparent background and border.
...
For ghost buttons, this matches the code-side state model.
...
## Composition rules
- Ghost is a borderless tertiary/supporting action treatment.
- Use filled + outline for explicit primary/secondary action rows.
- Do not pair ghost + ghost when one action is primary and the
  other is secondary.
...
- specs/figma-descriptions/button-icon-ghost.md
