...
Apply these rules to every generated or edited React component.
...
- Use visible `:focus-visible` styles with DLS focus ring tokens.
- Composite widgets follow WAI-ARIA Authoring Practices keyboard behavior.
- Modals trap focus, restore focus on close, and close with Escape.
- Escape closes overlays.
- Respect `prefers-reduced-motion` for transform, position, scale, or other spatial motion.

## Library And Styling Preferences
...
- DLS-Lead uses DLS CSS and tokens. Do not introduce Tailwind unless the target package already uses Tailwind.

## Testing

- Query elements with `getByRole` and accessible name.
