...
Category: action
Figma component set: icon-outline
React: <Button variant="outline" iconOnly>
...
Figma: https://www.figma.com/design/oKckNLXwLm5fYEmdXaynYB/DLS-Lead?node-id=6459-3036
...
This Figma component set uses: OUTLINE ICON-ONLY BUTTON.
Code mapping is fixed to `variant="outline"` and `iconOnly={true}`,
...
## Purpose
Compact square secondary action button containing only an icon,
with a transparent background and intent-colored border. Use for
dense toolbars, table row actions, icon clusters, and repeated
controls where the icon meaning is clear in context and code
provides an accessible name.

## Use when
- The action must fit in a compact square control.
- The icon-only action is secondary in its action group or toolbar.
- Pairing with a filled icon-only primary action.
- The action needs secondary outline emphasis rather than a filled
  surface.
...
- Primary icon-only actions in a primary/secondary pair -> use
  icon-filled.
- Rows or toolbars where another outline icon-only button already
  represents the secondary action and one action is meant to be
  primary. Do not create outline + outline icon action pairs.
- Primary page CTAs where a visible label is expected.
- Filled icon-only emphasis -> use icon-filled.
...
- This component set covers outline icon-only buttons only.
...
3. Border - intent-colored outline.
...
For outline icon-only buttons, this matches the code-side state
model.

### Code implementation
- normal -> transparent background, intent icon, intent border.
...
## Composition rules
- Outline icon-only is the secondary icon-action treatment.
