...
Category: action
Figma component set: icon-dotted
React: <Button variant="dotted" iconOnly>
...
Figma: https://www.figma.com/design/oKckNLXwLm5fYEmdXaynYB/DLS-Lead?node-id=6459-3310
...
This Figma component set uses: DOTTED ICON-ONLY BUTTON.
Code mapping is fixed to `variant="dotted"` and `iconOnly={true}`,
...
## Purpose
Compact square dashed-border action button containing only an icon.
Use for optional, additive, placeholder-like, or tentative icon
actions in dense toolbars, table row actions, and repeated action
surfaces where the icon meaning is clear in context and code provides
an accessible name.

## Use when
- The action must fit in a compact square control.
- The icon-only action is optional or supporting rather than the
...
- Secondary icon-only actions paired with a primary action -> use
  icon-outline.
- Rows where dotted + dotted icon buttons represent primary and
  secondary actions.
- Stronger tonal icon-only actions -> use icon-soft.
...
- This component set covers dotted icon-only buttons only.
...
3. Dashed border - intent-colored dashed outline.
...
For dotted icon-only buttons, this matches the code-side state model.

### Code implementation
- normal -> transparent background, intent icon, dashed intent border.
...
## Composition rules
- Dotted icon-only is a dashed supporting or optional icon-action treatment.
- Use filled icon-only + outline icon-only for explicit
  primary/secondary icon action pairs.
- Do not pair dotted + dotted icon buttons when one action is primary
