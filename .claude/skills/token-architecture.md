...
|-------|------|---------|---------|
| 1 | Primitives | Raw values | `color.neutral.700`, `radius.m`, `spacing.4` |
| 2 | Semantics | UI roles | `color.surface.base`, `color.intent.primary.base` |
| 3 | State | Behavioral | `state.lDelta.hover`, `state.disabled.opacity` |
| 4 | Component | Per-component | `color.component.button.primary.bg.base` |

References flow downward only: L4->L3/L2/L1. L3->L2/L1. L2->L1. L1->raw values.

## Naming Convention

Token path: `category.scope.element.variant.property.state` (lowercase, dots).
