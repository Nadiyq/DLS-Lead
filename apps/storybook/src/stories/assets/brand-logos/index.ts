 ...
 * PayPal, etc.) that intentionally use brand colors and lockups, not design
 * tokens. Lucide-only rules do not apply.
 *
 * Vite imports `.svg` files as URL strings by default; use them with `<img>`
 * or as a `background-image`, e.g.:
 *
 *   import mastercardLogo from '../assets/brand-logos/mastercard.svg';
